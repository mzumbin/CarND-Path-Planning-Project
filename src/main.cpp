...
std::tuple<double, double> rotate(const double &x, const double &y, const double &theta) {
    const double x_ = x * cos(theta) - y * sin(theta);
    const double y_ = x * sin(theta) + y * cos(theta);
...
int main() {
    ...
    int lane = 1;
    double dt = 0.02;
    auto get_xy = [&map_waypoints_s, &map_waypoints_x, &map_waypoints_y](const double &s, const double &d) {
        return getXY(s, d, map_waypoints_s, map_waypoints_x, map_waypoints_y);
    };

    double ref_vel = 0;
    h.onMessage(
            [&dt, &map_waypoints_x, &map_waypoints_y, &map_waypoints_s, &map_waypoints_dx, &map_waypoints_dy, &get_xy, &lane, &ref_vel](
                ...
                if (length && length > 2 && data[0] == '4' && data[1] == '2') {
                    ...
                    if (s != "") {
                        ...
                        if (event == "telemetry") {
                            ...
                            // insert a tangent path if lane has less then 2 points
                            if (prev_size < 2) {
                            ...
                            //change coordinates to car coordinates
                            ...
                            for (int i = 1; i <= 50 - previous_path_x.size(); i++) {
                                // adjust velocety in using an acceleration of 5m/s2
                                if (ref_vel > max_vel) {
                                    ref_vel -= 0.223;
                                } else {
                                    ref_vel += 0.223;
                                ...
                                double N = ((dt * ref_vel / 2.24) /
                                            target_dist); // porcetange of displacement in direction to target

                                double x_point =
                                        x_add_on + (target_x * N); // porcentage in x is proportional to the target
